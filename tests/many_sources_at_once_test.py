...
def test_all_inputs_at_once(mocker):
    ...
    input_args = InputArgs(prefix="ATRO_TEST", sources=[ArgSource.cli, Path(__file__).parent / "test.yaml", ArgSource.envs, Path(__file__).parent / "data_test_files" / ".env"])
...
def test_priority(mocker):
    ...
    input_args = InputArgs(
        ...
        sources=[
            ...
            ArgSource.envs,
            Path(__file__).parent / "data_test_files" / ".env",
        ],
    )
    input_args.add_arg(Arg(name="app_name", arg_type=str, help="App name", required=True))
    ...
    # Change priority
    sources = [ArgSource.envs, ArgSource.cli, Path(__file__).parent / "test.yaml", Path(__file__).parent / ".env"]
